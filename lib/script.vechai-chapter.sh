...
hxnormalize  -x $temp1 | hxselect "#zoomtext" | tr '\n' ' ' | sed 's/http/\nhttp/g' | grep "vechai.info" | sed -re 's/<\/(.*)//g' | sed -e 's/\/\".*\">/\/  /g' -e 's/html\".*\">/html  /g' -e 's/htm\".*\">/htm  /g' -e 's/<strong>//g' -e  's/\">.*\">/\/  /g'  | grep -v -e "hoi-nhung-nguoi-thich-doc-truyen-tranh\|hihuhi\|.asia\|<div\|class\|\"\|>\|<" | sed -re 's/-    (.*)//g;s/]//g;s/\[//g;s/tv(.*)//gI;s/\(//g;s/\)//g;s/ :(.*)//g;s/: (.*)//g' | sed -re 's/([0-9]*[0-9]+)/#\1#/g' | sed -re 's/(^.*#).*/\1/g' | sed 's/#//g' | awk '{gsub( /\./,"-",$NF);gsub( "_","-",$NF); print "#"$NF"#", $1}' | awk '{gsub("^#0*","#",$1); gsub("^##","#0#",$1); print $0}'  | grep -v -e "#http:\/\/"  > $output1
